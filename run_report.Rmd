...
list.of.packages <- c("rmarkdown", "httr", "jsonlite", "stringr", "knitr", "dplyr", "lubridate", "ggplot2", "ggthemes", "pander", "reshape", "summarytools")
new.packages <- list.of.packages[!(list.of.packages %in% installed.packages()[,"Package"])]
if(length(new.packages)) install.packages(new.packages)

# Loads the required packages
library(rmarkdown) #to generate the report
library(httr)     # for accessing the REST api
library(jsonlite) # for parson the JSON
library(stringr)  # not necessary but useful to format the column names automatically
library(knitr)     # for making the final word document
library(dplyr)     # for data manipulation
library(lubridate) # for date formatting
library(ggplot2)   # for graphics
library(pander)    # for markdown formatting
...
format_chart <- function(textang = 0, legend_name = "Legend") {
  list(
    theme_bw(),
...
#trim whitespace from before and after a column's values
## preserve column names in a vector until I can figure out why the / are replaced with .
names <- names(ona_df)
ona_df <- as.data.frame(lapply(ona_df, trimws))
names(ona_df) <- names
...
## Create lookup table to decode variable names into strings and assign weights
  lookup_csi <- data.frame(factor = c(copinga_food_lesspref, copinga_food_help, copinga_food_fewmeals, copinga_food_lessadult, copinga_food_mealsize), 
...
##### Example text follows, replace with your own details:

##### Data was collected using a standardized quantitative PDM questionnaire developed by Mercy Corps and modified to meet the neeeds of the XX program. The questionnaire, included as appendix 1, includes information about the distribution process, household profiles, expenditures, coping strategies, and main priorities. The selection of respondents for the PDM interviews was based on a simple random sample of the participants in each distribution. The sample size was calculated based on a XX% confidence level and X% margin of error. The targeted sample was X and the total number of respondents was `r nrow(ona_df)`.

## Data Collection and Analysis  

Data was collected and stored using the Ona software. Data was cleaned and analyzed using the R open-source statistical language, and a template for each report was written in RMarkdown. The template R script is stored and documented on [Github](github.com/xxxx).
...
## Findings on process (most recent distribution)

This section discusses findings about the quality of the most recent distribution process, including the process for beneficiary selection, respondents' understanding of the distribution process, and their experiences during the distribution and in transit.
...
### Respondent understanding of cash distribution
...
ggplot(filtered, aes(x = variable, y = value, fill = !!sym(vars$geo))) +
  ...
  labs(x = "", 
    ...
    scale_x_discrete(labels = c("Were you informed how \nmuch you will receive?", "Were you informed \nhow many times you \nwill receive the cash?", "Do you feel that you\nreceived the amount of\ncash that you were\nentitled to?")) + 
  coord_flip() +
  format_chart(legend_name = "District") +
  geom_text(aes(label = paste0(as.character(round(value, digits = 1)), "%")), position = position_dodge(width=0.9), hjust = 1, size = 2.5)

```

\newline

Of the respondents, `r round(sum(ona_df[[vars$informed_amount_yn]] == "yes")/nrow(ona_df), digits = 2)*100`% were informed of how much money they would receive, and `r round(sum(ona_df[[vars$informed_nodist_yn]] == "yes")/nrow(ona_df), digits = 2)*100`% were informed of how many times they would receive it. On average, respondents believed they would receive `r mean(ona_df[[vars$informed_amount]], na.rm = T)` YER in cash in `r mean(ona_df[[vars$informed_nodist]], na.rm = T)` distributions.
...
# replace data <NA> with real NA
ona_df[[vars$problems]] <- gsub("<NA>", NA, ona_df[[vars$problems]])
...
ona_df[[vars$costs_tosite]] <- as.numeric(as.character(ona_df[[vars$costs_tosite]]))
...
#### Figure 7: Percentage of respondents who received a receipt
```{r, echo = FALSE}

if(exists("distribution") == TRUE) {
  filtered <- select(ona_df, one_of(vars$distribution, vars$receipt_received, vars$household_status)) %>% 
    mutate_at(vars(vars$receipt_received), funs(lookup_yn$value[match(., lookup_yn$factor)])) %>%  
    group_by(!!sym(vars$distribution), !!sym(vars$geo)) %>% 
    summarise(receipt = sum(!!sym(vars$receipt_received))/n()*100)

  ggplot(filtered, aes(x = !!sym(vars$distribution), y = !!sym(vars$receipt), group = !!sym(vars$geo))) +
    geom_line(aes(color = !!sym(vars$geo))) +
    geom_point(aes(color = !!sym(vars$geo))) +
    labs(x = "", 
         y = "% of respondents") +
...
#### Figure 8: Costs to reach the distribution site
```{r, echo=F, message=F, warning=F}
...
### Respondent Profile (most recent PDM survey)

#### Table 3: Household Status of respondents, sex and age disaggregated
...
#### Figure 9: Respondent population profile
```{r, echo = FALSE}

filtered <- select(ona_df, one_of(vars$id, vars$hoh, vars$worked_pastmonth, vars$malnutrition, vars$breadwinner_lost, vars$breadwinner_nowork)) %>%   
  mutate_at(vars(one_of(vars$hoh, vars$worked_pastmonth, vars$malnutrition, vars$breadwinner_lost, vars$breadwinner_nowork)), funs(lookup_capitalize$value[match(., lookup_capitalize$factor)])) %>% 
  melt(., id.vars = vars$id)

ggplot(filtered, aes(x = rev(variable), fill = value)) +
  geom_bar() +
  labs(x = "", 
       y = "# of respondents") +
  scale_x_discrete(labels = c("Is the primary \nbreadwinner no \nlonger able to work?", "Has HH lost the \nprimary breadwinner?", "Anyone in family \ntreated for SAM?", "Has anyone in HH \nworked in past month?", "Are you the head \nof household?")) + 
  coord_flip() +
  theme_bw() +
  theme(axis.text.x = element_text(face = "bold", hjust = 1)) +
  scale_fill_pander(name = "Response")
...
* `r round(sum(ona_df[[vars$amount_spent]] == "100%")/nrow(ona_df), digits = 2)*100`% of respondents reported that they had spent all of the cash assistance, while `r round(sum(ona_df[[vars$amount_spent]] == "0%")/nrow(ona_df), digits = 2)*100`% reported that they had not spent any of it. The most frequent reason given for not spending all of the cash was "`r sort(table(ona_df[[vars$amount_spent_reasons]]), decreasing = TRUE)[1]`".
...
# replace data <NA> with real NA
ona_df[[vars$challenges]] <- gsub("<NA>", NA, ona_df[[vars$challenges]])
...
#### Figure 10: Needs covered by cash assistance
```{r, echo = FALSE}

# replace data <NA> with real NA
ona_df[[vars$proportion_needs_covered]] <- gsub("<NA>", NA, ona_df[[vars$proportion_needs_covered]])

ggplot(ona_df, aes(x = !!sym(vars$proportion_needs_covered), fill = !!sym(vars$proportion_needs_covered))) +
  geom_bar() +
  labs(x = "\nProportion covered", 
       y = "# of respondents",
...
#### Figure 11: Uses of cash assistance
```{r, echo=F, message=F, warning=F}
...
ggplot(filtered, aes(x = variable, y = value)) +
  ...
  scale_x_discrete(labels = strtrim(names_plot, width = 25)) +
  format_chart(textang = 45) +
  scale_color_pander() +
  theme(legend.position = "none")
...
### Coping Strategies Index (all distributions)

As noted in the World Food Programme's [Coping Strategies Index Manual](https://documents.wfp.org/stellent/groups/public/documents/manual_guide_proced/wfp211058.pdf), the basic logic of the Coping Strategies Index (CSI) is to help analysts understand the target population's answer(s) to the following question: "What do you do when you don’t have enough food, and don’t have enough money to buy food?" There are common short-term and long-term coping strategies that are relatively universal, and the prevalence of each can help to illustrate the severity of food insecurity in a target population.

From the manual: "The basic idea is to measure the frequency of these coping behaviors (how often is the coping strategy used?) and the severity of the strategies (what degree of food insecurity do they suggest?). Information on the frequency and severity is then combined in a single score, the Coping Strategies Index, which is an indicator of the household’s food security status."

The coping strategies used to construct the Food CSI in this report are listed here:
...
Calculation of the CSI score for each household was done according to the following equation:

$$HH CSI = \Sigma (# of days HH utilized CS * weight value of CS)$$

The maximum value of the CSI per household is `r 7*sum(1, 2, 1, 3, 1)`.

#### Figure 12: Coping Strategies Index scores disaggregated by location
```{r, echo=F, message=F, warning=F}
...
ggplot(scores, aes(x = !!sym(vars$geo), y = household_csi)) +
  ...
  geom_boxplot(aes(fill = !!sym(vars$geo)), colour = "black", width = 0.5,
               show.legend = FALSE, outlier.shape = NA, alpha = 0.4) +
  xlab("") +
  ylab(paste0("CSI Score (max = ", as.character(7*sum(1, 2, 1, 3, 1)), ")")) +
  theme_bw() +
...
#### Figure 13: Coping Strategies Index household scores classified by food insecurity severity
```{r, echo = FALSE}

# max score on CSI per household is 7 * (sum of all weights)
...
ApplyQuantiles <- function(x) {
  cut(x, breaks= c(0, 7, 14, 21, 7*sum(1, 2, 1, 3, 1)), 
...
# ggplot(scores, aes(x = distribution, y = percentage, fill = quantile)) +
#   geom_col() +
#   labs(x = "", 
#        y = "% of respondents",
#        title = "Figure X: Post-assistance Coping Strategies Index Scores (all distributions)",
...
ggplot(scores_quant, aes(y = percentage/100, x = !!sym(vars$geo), fill = quantile)) + 
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = "CSI scores among surveyed households") +
  theme(legend.position = "bottom",
...
If the program were to be redone, `r round(sum(ona_df[[vars$preferred_modality]] == "vouchers")/nrow(ona_df), digits = 2)*100`% of respondents would prefer to receive assistance in the form of vouchers, while `r round(sum(ona_df[[vars$preferred_modality]] == "the_equivalent")/nrow(ona_df), digits = 2)*100`% would prefer to receive the equivalent value in goods and services. `r round(sum(ona_df[[vars$preferred_modality]] == "cash")/nrow(ona_df), digits = 2)*100`% of respondents preferred to receive the cash.
...
pandoc.table(table(select(ona_df, one_of(vars$reason_modality_preferred, vars$preferred_modality))))
...
* `r round(sum(ona_df[[vars$feel_less_safe]] == "Yes")/nrow(ona_df), digits = 2)*100`% of respondents said they felt less safe or more at risk by receiving the assistance.
* `r round(sum(ona_df[[vars$community_tension]] == "yes")/nrow(ona_df), digits = 2)*100`% of respondents said their participation in the program caused tensions between them and the community.
* `r round(sum(ona_df[[vars$household_tension]] == "yes")/nrow(ona_df), digits = 2)*100`% of respondents said their participation in the program caused tensions between them and the household.
* `r round(sum(ona_df[[vars$program_exchange]] == "yes")/nrow(ona_df), digits = 2)*100`% of respondents said that in order to be included in the program, they had to pay a fee, make a favor in a return, get involved in suspicious work, or work for someone in some other way.

##### Insert any qualitative analysis about the respondents' assessment of their safety here.

#### Figure 14: Profile of respondent understanding of and participation in CARM procedures
```{r, echo = FALSE}

filtered <- select(ona_df, one_of(vars$id, vars$carm_aware_contact, vars$carm_would_report, vars$carm_ever_report)) %>%   
  mutate_at(vars(one_of(vars$carm_aware_contact, vars$carm_would_report, vars$carm_ever_report)), funs(lookup_capitalize$value[match(., lookup_capitalize$factor)])) %>%
  melt(., id.vars = vars$id)
...
ggplot(filtered, aes(x = rev(variable), fill = value)) +
  ...
  labs(x = "", 
       y = "# of respondents") +
  scale_x_discrete(labels = c("Have you ever used\none of our feedback\nmechanisms?",  "If you had a complaint\nor suggestion, would\nyou report it?", "Are you aware of \n how to contact us?")) + 
  coord_flip() +
  format_chart(legend_name = "Response")
  
```

\newline

Of all respondents, `r sum(ona_df[[vars$carm_ever_report]] == "yes")` said they had ever used one of Mercy Corps' feedback mechanisms. Of those, `r round(sum(ona_df[[vars$carm_received_response]] == "yes")/sum(ona_df[[vars$carm_ever_report]] == "yes"), digits = 2)*100`% said they received a response, and `r round(sum(ona_df[[vars$carm_satisfied_response]] == "yes")/sum(ona_df[[vars$carm_ever_report]] == "yes"), digits = 2)*100`% said they were satisfied with the response.

\newline

#### Figure 15: Preferred methods for CARM reporting
```{r, echo = FALSE}

ona_df[[vars$carm_prefer_report]] <- gsub("<NA>", NA, ona_df[[vars$carm_prefer_report]])
...
ggplot(filter(ona_df, !is.na(!!sym(vars$carm_prefer_report))), aes(x = !!sym(vars$carm_prefer_report), fill = !!sym(vars$carm_prefer_report))) +
  ...
  facet_grid(eval(expr(. ~ !!ensym(geo)))) +
  format_chart(textang = 45) +
  theme(legend.position = "none")
