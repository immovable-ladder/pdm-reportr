...
list.of.packages <- c("rmarkdown", "httr", "jsonlite", "stringr", "knitr", "dplyr", "lubridate", "ggplot2", "ggthemes", "pander", "reshape", "summarytools", "here")
...
# GET the information from Ona, using the form_id, username and password params
  #ona_raw_request <- GET(paste0("https://api.ona.io/api/v1/data/", params$form_id), 
                         ona_raw_request <- GET(paste0("https://api.ona.io/api/v1/data/", "406847"), 
                        authenticate("hannalcamp", "ww123ona"))
...
Calculation of the rCSI score for each household was done according to the following equation:

$$HH rCSI = \Sigma (# of days HH utilized CS * weight value of CS)$$

The maximum value of the rCSI per household is `r 7*sum(1, 2, 1, 3, 1)`.
...
ggplot(scores, aes(x = !!sym(vars$geo), y = household_csi)) +
  ...
  geom_boxplot(aes(fill = !!sym(vars$geo)), colour = "black", width = 0.5,
               show.legend = FALSE, outlier.shape = NA, alpha = 0.4) +
  xlab("") +
  ylab(paste0("rCSI Score (max = ", as.character(7*sum(1, 2, 1, 3, 1)), ")")) +
  theme_bw() +
...
ggplot(scores_quant, aes(y = percentage/100, x = !!sym(vars$geo), fill = quantile)) + 
  ...
  labs(x = NULL, y = NULL,
       ...
       subtitle = "rCSI scores among surveyed households") +
  theme(legend.position = "bottom",
